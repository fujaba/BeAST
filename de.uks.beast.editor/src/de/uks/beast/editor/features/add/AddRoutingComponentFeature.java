...
public class AddRoutingComponentFeature extends AbstractAddFeature
{
	...
	public AddRoutingComponentFeature(final IFeatureProvider fp)
	{
		super(fp);
	}
	...
	{
		if (context.getNewObject() instanceof RoutingComponent)
		{
	...
	{
		final RoutingComponent routingComponent = (RoutingComponent) context.getNewObject();
		...
		if (routingComponent.eResource() == null)
		{
			getDiagram().eResource().getContents().add(routingComponent);
		}
		// create link and wire it
		link(containerShape, routingComponent);
		...
		final Text text = gaService.createText(textShape, routingComponent.getType());
		...
		// create link and wire it
		link(textShape, routingComponent);
